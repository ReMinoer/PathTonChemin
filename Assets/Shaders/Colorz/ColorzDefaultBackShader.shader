Shader "Colorz/DefaultBack"
{
	Properties
	{
		...
		_RimSizeEdge("Rim Size Edge", Range(0.0,2.0)) = 0.5
	}
	...
	SubShader
	{
		...
		CGPROGRAM
			...
			sampler2D _MainTex, _OccTex;
			float _OccPower, _RimPower, _RimCP, _RimCL, _RimPowerEdge, _RimSizeEdge;
		...
		CGPROGRAM
			...
			void surf (Input IN, inout SurfaceOutput o)
			{
				...
				half3 rimNormal = pow((((saturate(dot(normalize(IN.viewDir), o.Normal))) - _RimCP) * _RimCL) + _RimCP, 1-_RimPower);
				// Cull FRONT
				half rimEdge = saturate(dot(normalize(IN.viewDir), o.Normal));
				o.Emission = occ.rgb * (tex.rgb * rimNormal + ((tex.rgb*(1-_RimPowerEdge) + occ.rgb*_RimPowerEdge) * rimEdge));
			}
		ENDCG
	}
